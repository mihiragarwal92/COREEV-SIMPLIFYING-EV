...
(function( $ ) {
	...
	$( document ).ready(function() {
		...
		if( o_stat ){
			...
			var header_offset = explug_ajax_var.header_offset;
			var mheader_offset = explug_ajax_var.mheader_offset;
			var res_width = explug_ajax_var.res_width;
		...
		if( $('.zozo-mc').length ){
			$('.zozo-mc').on( "click", function () {
				...
				if( mc_form.find('input[name="zozo_mc_email"]').val() == '' ){
					mc_wrap.find('.mc-notice-msg').text( explug_ajax_var.must_fill );
				}else{
					c_btn.attr( "disabled", "disabled" );
					$.ajax({
						type: "POST",
						url: explug_ajax_var.ajax_url,
						data: 'action=zozo-mc&nonce='+explug_ajax_var.mc_nounce+'&'+mc_form.serialize(),
	...
	$( window ).load(function() {
		...
		//Explug masonry
		if( $(".explug-masonry").length ){
			var masonry_on_time;			
			var masonry_cols = $(".explug-masonry").data('columns');			
			var masonry_gutter = $(".explug-masonry").data('gutter');
			
			$(".explug-masonry").rcmasonry({
    ...
    //Elementor RTL issue
    function explug_for_elementor_row(){
    ...
    $(window).resize(function(event){
		...
		if( $(".sticky-outer").length ){
			$(".sticky-outer").each(function(){
				...
				_sticky_ele.explug_stickypart();
				var sticky_on_time;
				$(window).resize(function(event){
					clearTimeout(sticky_on_time);
					sticky_on_time = setTimeout(function(){ _sticky_ele.explug_stickypart(); }, 300);
				});
			});
		}
...
(function ( $ ) {
	...
	//Make sticky
	$.fn.explug_stickypart = function( options ){
